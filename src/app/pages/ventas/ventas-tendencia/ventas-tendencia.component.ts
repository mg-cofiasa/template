...
import { VentasTendenciaService } from '../services/ventas-tendencia.service';
...
@Component({
  ...
  templateUrl: './ventas-tendencia.component.html',
  styleUrls: ['./ventas-tendencia.component.scss'],
  providers: [MessageService]
})
export class VentasTendenciaComponent {
  ...
  public cboTipoEjercicioData: iCatalogoIdNombre[] = [];
  ...
  constructor(
    ...
    private sharedService: SharedService
      ...
      for(let anio = (Number(this.fechaHoy.getFullYear())) ; anio >= Number(this.fechaHoy.getFullYear()-5) ; anio--){
        ...
        this.cboPeriodoData.push({Nombre: anio + ""});
      }

      this.cboTipoEjercicioData.push({Id: 1, Nombre: "Año móvil"});
      this.cboTipoEjercicioData.push({Id: 2, Nombre: "Año calendario"});
  ...
  public GetDatosGraficas(tipo: string, event: any = null, graficaTabla: string){    
    ...
    let periodo: iCatalogoNombre = <iCatalogoNombre>{};
    this.enviarDatosGrafica.VentaNeta = this.form["chkVentaNeta"].value;

    if (this.form["cboTipoEjercicio"].value === null){
      this.messageService.add({severity: 'info', summary: 'Seleccionar un ejercicio', detail: `Seleccionar un ejercicio de la lista`});
      return 1;
    }
    else if (this.form["cboTipoEjercicio"].value.Id === 2){
      if (this.form["cboPeriodo"].value === null){
        ...
        periodo = this.form["cboPeriodo"].value;
      }
    }
    else{
      periodo = {Nombre: '1'}
  ...
  public MostrarGraficas(enviarDatosGrafica: iEnviarDatosGrafica, actualizarDatos: boolean){
    ...
    this.ventasTendenciaService.GetDatosGraficasTendencia(enviarDatosGrafica).subscribe({
      next: (Respuesta: any) => {
        ...
        if (this.form["chkProyeccionUtilidad"].value === false){
          ...
          let datosGraficaMezcla: iEnviarDatosGrafica = {
            Periodo: enviarDatosGrafica.Periodo,
            Segmento: '',
            TipoLugar: enviarDatosGrafica.TipoLugar,
            LugarValor: enviarDatosGrafica.LugarValor,
            Categoria: '',
            VentaNeta: enviarDatosGrafica.VentaNeta,
            ProyeccionUtilidad: true,
            Concepto: ''
          }

          if (actualizarDatos === true){
            this.ventasService.GetDatosActualizadosGraficas(datosGraficaMezcla).subscribe({
              next: (RespuestaMezcla: iArregloGraficaDatos) => {
                Swal.close();
                this.actualizacionInformacion = new Date();
                this.messageService.add({severity: 'success', summary: 'Información actualizada', detail: `La consulta se realizó correctamente`, key: 'preload_success', life:1000});

                //this.GenerarGraficaMezcla(RespuestaMezcla);
            ...
            this.ventasService.GetDatosGraficas(datosGraficaMezcla).subscribe({
              next: (RespuestaMezcla: iArregloGraficaDatos) => {
                Swal.close();
                this.messageService.add({severity: 'success', summary: 'Información actualizada', detail: `La consulta se realizó correctamente`, key: 'preload_success', life:1000});
  ...
  public GenerarGraficasProyeccion(Respuesta: any){
    if (Respuesta.UtilidadOperacion !== undefined){
      ...
      this.faltantePuntoEquilibrioGrafica.title = {text:"FALTANTE PUNTO EQUILIBRIO"};
  ...
  public MostrarTabla(enviarDatosGrafica: iEnviarDatosGrafica, actualizarDatos: boolean){
    ...
    this.ventasTendenciaService.GetDatosTablaTendencia(enviarDatosGrafica).subscribe({
      next: (Respuesta: any[]) => {
        Swal.close();
        this.actualizacionInformacion = new Date();
        this.messageService.add({severity: 'success', summary: 'Información actualizada', detail: `La consulta se realizó correctamente`, key: 'preload_success', life:1000});
  ...
  public LLamarGraficaMezcla(){
    let iEnviarDatosGrafica: iEnviarDatosGrafica = {
      Periodo: '202309',
      Segmento: '',
      TipoLugar: 'Plaza',
      LugarValor: 'chihuahua',
      Categoria: '',
      VentaNeta: false,
      ProyeccionUtilidad: false,
      Concepto: ''
    }

    this.MostrarGraficaMezcla(iEnviarDatosGrafica, false);
  }

  /**
   * Acción: Obtiene información para generar las gráficas
   */
  public MostrarGraficaMezcla(enviarDatosGrafica: iEnviarDatosGrafica, actualizarDatos: boolean){
    this.sharedService.SwalModalAwaitServer();
    ...
    if (actualizarDatos === true){
      this.ventasService.GetDatosActualizadosGraficas(enviarDatosGrafica).subscribe({
        next: (Respuesta: iArregloGraficaDatos) => {
          ...
          this.GenerarGraficaMezcla(Respuesta);
        },
        error: (error) => {
        }
      });    
  ...
  public GenerarGraficaMezcla(Respuesta: iArregloGraficaDatos){
    ...
    this.mezclaGrafica.title = {text:"MEZCLA"};
    this.mezclaGrafica.yAxis = {title: {text: "TONELADAS / PORCENTAJE"}};    
  } 
}